...
在~/platon-node目录下创建创世区块配置文件platon.json，然后将以下的创世区块配置文件模板内容拷贝到platon.json文件中，修改 `your-node-pubkey` 为之前生成的***节点公钥(nodekey)***，`your-node-blspubkey` 为 ***节点bls公钥(blspub)***，`your-account-address`为 ***创建钱包的地址(Address)***：
