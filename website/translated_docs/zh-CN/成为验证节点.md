...
> 注意：
>
> 解压 MTool 工具包安装完后需重启命令行让 MTool 环境变量生效
...
如果共识节点部署完成，并且节点已经追上 [Platscan](https://platscan.test.platon.network) 网站上的块高，您就可以使用 MTool 进行质押操作。质押资金申请完成后，确保质押账户余额足够，根据用户情况替换质押金额，质押最低门槛为100万 LAT。
