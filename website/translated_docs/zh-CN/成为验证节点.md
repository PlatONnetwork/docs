...
> - nodeName: 节点名称。
> - details: 节点描述信息。
> - externalId: 对应为keybase的`PGP key`，PlatScan通过此值显示为对应的头像；如果用户需要在PlatScan显示指定的头像，可在 [keybase.io](https://keybase.io/) 官网进行注册，然后在keybase上传头像，并将externalId的字段值修改成PGP key，即16位的公钥 ，如
> - webSite: 公司网址。
...
如果共识节点部署完成，并且已经同步区块成功，您就可以使用 MTool 进行质押操作。质押资金申请完成后，确保质押账户余额足够，根据用户情况替换质押金额，质押最低门槛为100万 LAT。

注意：请保持质押账户里面有足够 LAT，以备后续发起节点管理的交易有足够的交易手续费，比如升级提案的投票，解质押等交易。
