...
如果共识节点部署完成，并且节点已经追上 [PlatScan](https://platscan.platon.network) 网站上的块高，您就可以使用 PlatON MTool 进行质押操作。质押操作前请确确保质押账户余额足够，质押最低门槛为10万 LAT。接入开发网可向社区申请质押所需要的LAT，请您按照格式要求发送邮件至 support@latticex.foundation，邮件要求：
