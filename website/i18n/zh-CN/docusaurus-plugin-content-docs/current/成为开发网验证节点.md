...
```bash
sudo wget https://download.platon.network/platon/devnet/platon/1.1.1/platon -O /usr/bin/platon
sudo wget https://download.platon.network/platon/devnet/platon/1.1.1/platonkey -O /usr/bin/platonkey
...
开发网为开发者或节点提供开发测试环境。可能出现不稳定，网络重置的情况。开发网络目前版本为1.1.1。
