...
计算一个PlatON特定的签名： sign(keccak256("Ethereum Signed Message: " + len(message) + message))。
...
获取指定共识轮的验证人列表，传入某个共识轮中的任意区块高度，系统自动计算块高所属共识轮，返回该共识轮的验证人列表，无数据时返回空字符串。

##### 参数
Uint64 - 区块高度

##### 返回
`array` - 验证人列表，Address：节点地址；NodeId：节点Id；BlsPubKey：节点bls公钥。
