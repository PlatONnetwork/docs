...
class SideNav extends React.Component {
        ...
        <ul className={ulClassName}>
          {categoryItem.children.map((item) => {
            switch (item.type) {
              ...
              case 'SONCATEGORY':
                return this.renderSONCategory(item);
              default:
                return null;
            }
          })}
  ...
  renderSONCategory = (subcategoryItem) =>(
    <div className="navGroup subNavGroup sonNavGroup" key={subcategoryItem.title}>
      <h4 className="navGroupSubcategoryTitle sonNavTitle">
        {this.getLocalizedCategoryString(subcategoryItem.title)}
      </h4>
      <ul>{subcategoryItem.children.map(this.renderItemLink)}</ul>
    </div>
