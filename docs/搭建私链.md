...
Create the creation block configuration file platon. Json in the `~/platon-node` directory, then copy the following creation block configuration file template content to platon. Json file, modify `your-node-pubkey` as the previously generated ***node public key (nodeid)***, `your-node-blspubkey` as ***node BLS public key (blspub)***, `your-account-address` as ***the wallet address*** of generate wallet file:
