...
| --db.nogc      | Enable archive mode, when this switch is turned on, the garbage collection mechanism will no longer work, so the CPU overhead will be reduced and the responsive disk overhead will be increased                                       |
