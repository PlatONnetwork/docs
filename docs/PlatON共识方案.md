...
Assume each view allows $$n$$ blocks to be generated. The current view $$ V*i $$ time window expires and the view switches to $$ V* {i+1} $$. At this time, only part of blocks generated by $$ V_i $$ get QC, and some blocks will be reorganized, the reorganization rules are as follows:
