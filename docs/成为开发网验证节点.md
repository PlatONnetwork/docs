...
sudo wget https://download.platon.network/platon/devnet/platon/1.2.1/platonkey -P /usr/bin
```

devnet2:

```bash
sudo wget https://download.platon.network/platon/devnet2/platon/1.2.2/platon -O /usr/bin/platon
sudo wget https://download.platon.network/platon/devnet2/platon/1.2.2/platonkey -O /usr/bin/platonkey
```
...
The PlatON development network 1, which will be open to developers, is set to launch on March 10, 2021 (Chainid: 210309).
The PlatON development network 2, which will be open to developers, is set to launch on June 13, 2022 (Chainid: 2206131).
...
The development network provides a development test environment for the developer or node.There may be instability and a network reset.The current version of the development network is 1.2.1.

#### Develop network related resources

>- platon：devnet1：https://download.platon.network/platon/devnet/platon/1.2.1/platon
           devnet2：https://download.platon.network/platon/devnet2/platon/1.2.2/platon
>- platonkey：devnet1：https://download.platon.network/platon/devnet/platon/1.2.1/platonkey
              devnet2：https://download.platon.network/platon/devnet2/platon/1.2.2/platonkey
>
...
>
>  > You need to change the chain ID in the configuration file config.properties to the development network 1 chain ID: 210309；
>  > You need to change the chain ID in the configuration file config.properties to the development network 2 chain ID: 2206131；
>
> - samurai：
> - Open the RPC URL:http://35.247.155.162:6789 and ws://35.247.155.162:6790
> - development network 1 Scan Browser Address: https://devnetscan.platon.network
...
```bash
cd ~/platon-node && wget https://download.platon.network/platon/devnet2/platon/1.2.2/genesis.json
```
...
```shell
cd ~/platon-node/ && nohup platon --identity platon-node --datadir ./data --port 16789 --rpcport 6789 --rpcapi "db,platon,net,web3,admin,personal" --rpc --nodekey ./data/nodekey --cbft.blskey ./data/blskey --verbosity 1 --rpcaddr 127.0.0.1 --bootnodes enode://4ea24accba3da480021bd8c9699e702d9bec0e5f3d7dd1a354697e83b67a4c275b3fb2a21a3be2438a3430fd4b8a9f97acf1c090c1ade599a619fedeac1fb730@devnet2node1.platon.network:16789 --syncmode "fast" > ./data/platon.log 2>&1 &
```
...
If you want to run mtool on the development network 1, change the chain ID in the config.properties file to 210309；
If you want to run mtool on the development network 2, change the chain ID in the config.properties file to 2206131；
