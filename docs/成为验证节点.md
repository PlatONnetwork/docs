...
- Staking wallet: The staking wallet is used to staking tokens. To become a candidate node, you must stake successfully. Run the following command to create a stake  wallet:
...
Note: Please keep enough LAT in the stake account, so that the transactions initiated by the subsequent node management have sufficient transaction fees, such as voting for upgrade proposals, and unsecured transactions.
