...
- Staking wallet: The staking wallet is used to stake tokens. To become a candidate node, you must stake successfully. Run the following command to create a stake  wallet:

```bash
mtool-client account new staking
```

Enter the password once and confirm the password again to create a wallet file. After the creation is successful, a stake wallet file `staking.json` will be generated in the directory` $MTOOLDIR/keystore`.
...
> - externalId: Corresponding to the `PGP key` of keybase, PlatScan displays the corresponding avatar through this value; if the user needs to display the specified avatar on PlatScan, he can register on the official website of  [keybase.io](https://keybase.io/), then upload the avatar on keybase, and modify the field value of externalId `PGP key`, which is a 16-bit public key.
